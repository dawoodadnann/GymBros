...
public class LeaderboardController : ControllerBase
{
    ...
    public async Task<ActionResult> GetLeaderboard() =>
        Ok(await _service.GetLeaderboard());

    [HttpGet("roast/{id1}/vs/{id2}")]
    public async Task<ActionResult> Roast(int id1, int id2)
    {
        var bro1 = await _service.GetById(id1);
        var bro2 = await _service.GetById(id2);
